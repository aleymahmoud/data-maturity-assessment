...
function ResumeByCodePageContent() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      router.push(`/dma/questions?question=${data.questionNumber}&code=${code}&lang=en`);
    } catch (error) {
    ...
    <div className="flex flex-col items-center justify-center min-h-screen p-4">
      <div className="w-full max-w-md p-6 bg-white rounded-lg shadow-md">
        ...
        <div className="mt-4 text-center">
          ...
          <Link href={`/dma/resume?lang=en`} className={`mx-1 ${language === 'en' ? 'font-bold' : ''}`}>English</Link> |
          <Link href={`/dma/resume?lang=ar`} className={`mx-1 ${language === 'ar' ? 'font-bold' : ''}`}>العربية</Link>
        </div>
      </div>
    </div>
  );
