...
function RoleSelectionPageContent() {
  ...
  const [questionCount, setQuestionCount] = useState(null); // Start with null
  ...
  const getContent = () => {
    const questionText = questionCountLoading ? 'loading...' : questionCount;
    const content = {
      en: {
        language: 'Language',
        title: 'Select Your Role',
        description: `Your role helps us provide personalized recommendations and relevant action plans. All users will answer the same ${questionText} questions regardless of role selection.`,
        languageSection: 'Assessment Language:',
        changeLanguage: 'Change language:',
        note: 'Note:',
        noteText: `Role selection is used only for personalizing your recommendations. You will see all ${questionText} questions covering the complete data maturity framework.`,
        ...
        warningText: `If you close the assessment before answering all ${questionText} questions, you will lose your progress and need to restart the assessment.`,
        roles: {
          executive: {
      ...
      ar: {
        language: 'اللغة',
        title: 'اختر دورك',
        description: `دورك يساعدنا في تقديم توصيات مخصصة وخطط عمل ذات صلة. جميع المستخدمين سيجيبون على نفس الـ ${questionText} سؤالاً بغض النظر عن اختيار الدور.`,
        languageSection: 'لغة التقييم:',
        changeLanguage: 'تغيير اللغة:',
        note: 'ملاحظة:',
        noteText: `يُستخدم اختيار الدور فقط لتخصيص توصياتك. سترى جميع الأسئلة الـ ${questionText} التي تغطي إطار عمل نضج البيانات الكامل.`,
        ...
        warningText: `إذا أغلقت التقييم قبل الإجابة على جميع الأسئلة الـ ${questionText}، ستفقد تقدمك وستحتاج لبدء التقييم من جديد`,
        roles: {
          executive: {
...
useEffect(() => {
  ...
  const isResuming = sessionStorage.getItem('isResuming') === 'true';

  if (!userData) {
    router.push(`/dma/user-info?lang=${language}`);
    return;
  }

  // If resuming, get role from database
  if (isResuming && userData) {
    try {
      const data = JSON.parse(userData);
      console.log('FULL userData contents:', data);
      setUserId(data.userId); // Store userId for API calls

      console.log('Resume data selectedRole:', data.selectedRole);

      // Use database stored role if available
      if (data.selectedRole) {
        console.log('Using database stored role:', data.selectedRole);
        setSelectedRole(data.selectedRole);

        // Auto-redirect to assessment if user already has a role selected
        setTimeout(() => {
          router.push(`/dma/questions?lang=${language}&role=${data.selectedRole}&question=0`);
        }, 1000); // Small delay to show the selected role briefly
...
const handleContinue = async () => {
  if (selectedRole) {
    ...
    router.push(`/dma/questions?lang=${language}&role=${selectedRole}&question=0`);
  }
};
              ...
              <div style={{ 
                ...
                <select 
                  ...
                  onChange={(e) => {
                    ...
                    router.push(`/dma/role-selection?lang=${newLang}`);
                  }}
                  style={{ 
                    padding: '6px 12px', 
                    borderRadius: '6px', 
      ...
      >
        <div style={{ 
          ...
          <div style={{ textAlign: 'center', marginTop: '20px' }}>
            <Link href={`/dma/user-info?lang=${language}`} style={{ 
              color: 'var(--text-secondary)', 
              textDecoration: 'none',
              fontFamily: 'var(--font-primary)'
            }}>
