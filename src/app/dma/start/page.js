  ...
  try {
    ...
    if (result.valid) {
      ...
      if (result.isCompleted) {
        ...
        // Include role in the URL if available
        const roleParam = result.userData?.selectedRole ? `&role=${result.userData.selectedRole}` : '';
        router.push(`/dma/results?lang=${language}&session=${result.sessionId}${roleParam}`);

      } else if (result.hasUserData && result.existingUser) {
        // User data exists - resume assessment directly
        const userData = result.existingUser;

        // Store resume data in session storage
        sessionStorage.setItem('userData', JSON.stringify(userData));
        sessionStorage.setItem('isResuming', 'true');

        // IMPORTANT: Store assessment code for questions API
        sessionStorage.setItem('assessmentCode', code.trim().toUpperCase());

        // Redirect directly to assessment with resume parameters
        router.push(`/dma/questions?lang=${language}&resume=true`);
