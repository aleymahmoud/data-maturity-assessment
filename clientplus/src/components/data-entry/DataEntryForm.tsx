...
export default function DataEntryForm({ onEntriesUpdated }: { onEntriesUpdated?: () => void }) {
  ...
  const fetchUserDomains = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setDomains(data)
        console.log('Loaded user domains:', data.length)
      } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(errorData.error || 'Failed to submit entries')
